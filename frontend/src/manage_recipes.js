...
const handleRecipeImageChange = (e) => {
  ...
  try {
    ...
    // ✅ CORRECT ENDPOINT
    const response = await fetch("http://127.0.0.1:8000/api/my-recipes/", {
      headers: { 
...
const handleEditSubmit = async (e) => {
  ...
  try {
    const token = getJwtToken();
    const response = await fetch(`http://127.0.0.1:8000/api/recipes/${editFormData.id}/update/`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
      },
...
const handleDeleteSubmit = async () => {
  try {
    const token = getJwtToken();
    const response = await fetch(`http://127.0.0.1:8000/api/recipes/${editFormData.id}/delete/`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
